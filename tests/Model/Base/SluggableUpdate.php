...
/**
 * Base class of the Model\SluggableUpdate entity.
 */
abstract class SluggableUpdate implements \ArrayAccess
{
    protected $id;
    ...
    public function updateSluggableSlug()
    {
        $slug = $proposal = call_user_func(array('\Doctrator\Behavior\Sluggable', 'slugify'), $this->getTitle());

        $similarSlugs = array();

        $query = $this->getEntityManager()->createQuery("SELECT e.id, e.slug FROM Model\SluggableUpdate e WHERE e.slug LIKE ?1");
    ...
    {
        return $this->getEntityManager()->getRepository('Model\SluggableUpdate');
    }
    ...
    static public function loadMetadata(\Doctrine\ORM\Mapping\ClassMetadata $metadata)
    {
        ...
        $metadata->setCustomRepositoryClass('Model\SluggableUpdateRepository');
