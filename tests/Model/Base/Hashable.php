...
/**
 * Base class of the Model\Hashable entity.
 */
abstract class Hashable implements \ArrayAccess
{
    protected $id;
    ...
    public function updateHashableHash()
    {
        do {
            $hash = '';
            for ($i = 1; $i <= 20; $i++) {
            ...
            $results = $this->getEntityManager()->createQuery("SELECT e.id FROM Model\Hashable e WHERE e.hash = '$hash'")->getArrayResult();
        } while ($results);
    ...
    {
        return $this->getEntityManager()->getRepository('Model\Hashable');
    }
    ...
    static public function loadMetadata(\Doctrine\ORM\Mapping\ClassMetadata $metadata)
    {
        ...
        $metadata->setCustomRepositoryClass('Model\HashableRepository');
